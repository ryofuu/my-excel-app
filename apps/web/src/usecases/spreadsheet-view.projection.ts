...
export type CalculatedWorkbookState = Readonly<{
  workbook: Workbook;
...
export const workbookView = (
  ...
  for (const [id, cell] of state.workbook.revision.cells) {
  ...
  return {
    ...
    worksheets: state.workbook.revision.worksheets.map((worksheet) => ({
      id: String(worksheet.id),
      name: String(worksheet.name),
    })),
    activeWorksheetId: String(worksheetId),
    revision: Number(state.workbook.revision.number),
    cells,
...
export const calculationInspection = (
  ...
  const id = cellId(worksheetId, address);
  const cell = state.workbook.revision.cells.get(id);
