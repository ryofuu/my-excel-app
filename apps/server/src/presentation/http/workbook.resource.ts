...
export const workbookUpdateResourceSchema = z.object({
  workbook: workbookResourceSchema,
  expectedRevision: z.number().int().nonnegative(),
});

export const workbookPathSchema = z.object({ id: z.string().min(1) });

export type WorkbookResource = z.infer<typeof workbookResourceSchema>;
export type WorkbookUpdateResource = z.infer<
  typeof workbookUpdateResourceSchema
>;
...
export const workbookFromResource = (resource: WorkbookResource): Workbook => {
  // Zod 通過後も各値を Domain Factory に通し、Transport の妥当性と業務上の妥当性を分離する。
  ...
  // 最後に Aggregate を生成し、Worksheet と Cell の参照整合性まで一括で検証する。
  return new Workbook({
